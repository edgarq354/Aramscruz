package com.elisoft.aramscruz.chat;
...
public class Chat extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fb_send = (FloatingActionButton) findViewById(R.id.send);
        listView = (ListView) findViewById(R.id.msgview);
        et_mensaje = (EditText) findViewById(R.id.msg);
    ...
    private boolean enviar_mensaje() {
        if(et_mensaje.getText().toString().trim().length()>0) {
            chatArrayAdapter.add(new CMensaje(side, et_mensaje.getText().toString(), perfil.getString("nombre", ""), now(), hora(), 0, Integer.parseInt(perfil.getString("id_usuario", "0")), id_conductor, 1));
        }
        et_mensaje.setText("");
...
try {
    ...
    Cursor fila = bd.rawQuery("select id,id_conductor,id_usuario,titulo,mensaje,fecha,hora,estado,yo from chat where id_usuario=" + id_usuario + " and id_conductor=" + id_conductor + " order by id asc ", null);
    ...
    if (fila.moveToFirst()) {  //si ha devuelto 1 fila, vamos al primero (que es el unico)
        ...
        do {
            ...
            chatArrayAdapter.add(new CMensaje(sw_left, mensaje, titulo, fecha, hora, Integer.parseInt(estado), Integer.parseInt(id_usu), Integer.parseInt(id_cond), Integer.parseInt(yo)));
